...
const FloatingLabelInput = ({ id, label, type, value, onChange, hasError }) => (
  <div className="relative">
    <input
      ...
      onChange={onChange}
      placeholder=" "
      className={`peer w-full rounded-lg border px-3 pt-5 pb-2 text-sm text-gray-900 placeholder-transparent focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 ${
    ...
    <label
      htmlFor={id}
      className="absolute left-3 top-2 text-gray-500 text-sm transition-all peer-placeholder-shown:top-5 peer-placeholder-shown:text-gray-400 peer-placeholder-shown:text-sm peer-focus:top-2 peer-focus:text-red-500 peer-focus:text-sm"
    >
      {label}
    </label>
  </div>
