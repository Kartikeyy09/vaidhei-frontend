// ✅ FILE: src/admin/components/journey/MilestoneEditor.jsx
...
const FloatingLabelTextarea = ({ id, label, rows = 4, ...props }) => (
    <div className="relative">
...
const MilestoneEditor = ({ isOpen, onClose, onSave, milestone, isSaving }) => {
    ...
    const handleSubmit = (e) => {
        ...
        dataToSave.append('year', formData.year);
        dataToSave.append('title', formData.title);
        dataToSave.append('description', formData.description);
        dataToSave.append('outcomes', formData.outcomes); // Send as comma-separated string as backend expects

        if (imageFile) {
            // IMPORTANT: This key 'image' must match your backend's multer middleware
            // e.g., upload.single('image')
            dataToSave.append('imageUrl', imageFile);
        }
                            ...
                            <Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-white p-8 text-left align-middle shadow-xl transition-all">
                                ...
                                <form onSubmit={handleSubmit} className="mt-6 space-y-6 max-h-[75vh] overflow-y-auto pr-4">
                                    <div onClick={() => fileInputRef.current.click()} className={`mt-2 flex justify-center rounded-lg border-2 border-dashed border-gray-300 px-6 py-10 cursor-pointer hover:border-red-500 ${imagePreview ? 'p-0 border-solid' : ''}`}>
                                        {imagePreview ? <img src={imagePreview} alt="Preview" className="w-full h-48 object-cover rounded-lg"/> : <div className="text-center"><PhotoIcon className="mx-auto h-12 w-12 text-gray-300" /><p className="mt-4 text-sm">Click to upload image</p></div>}
                                    </div>
                                    <input ref={fileInputRef} type="file" onChange={handleFileChange} className="hidden" accept="image/*"/>
                                    
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <FloatingLabelInput id="year" name="year" label="Year (e.g., 2024)" value={formData.year} onChange={handleChange} />
                                        <FloatingLabelInput id="title" name="title" label="Title" value={formData.title} onChange={handleChange} />
                                    </div>
                                    
                                    <FloatingLabelTextarea id="description" name="description" label="Description" value={formData.description} onChange={handleChange} />
                                    <FloatingLabelTextarea id="outcomes" name="outcomes" label="Key Outcomes (comma-separated)" value={formData.outcomes} onChange={handleChange} />
                                    ...
                                    <div className="pt-4 flex justify-end gap-4 sticky bottom-0 bg-white/80 backdrop-blur-sm pb-1 -mx-8 px-8">
                                        ...
                                        <button type="submit" className="px-6 py-2 rounded-md text-white bg-red-600 hover:bg-red-700 font-semibold disabled:opacity-50" disabled={isSaving}>{isSaving ? 'Saving...' : 'Save Milestone'}</button>
                                    </div>
                                </form>
                            </Dialog.Panel>
                        </Transition.Child>
