// ✅ FILE: src/admin/components/tenders/TenderEditor.jsx
...
const TenderEditor = ({ isOpen, onClose, onSave, tender, isSaving }) => {
    const [formData, setFormData] = useState({ title: '', tenderId: '', client: '', status: 'Active', dueDate: '' });
    ...
    useEffect(() => {
        if (isOpen) {
            if (tender) { // Edit Mode
                ...
                setDocumentFile(null); // Reset file input on open
            } else { // Add New Mode
                setFormData({ title: '', tenderId: '', client: '', status: 'Active', dueDate: '' });
                setDocumentUrl('');
    ...
    const handleSubmit = (e) => {
        ...
        if (documentFile) {
            // IMPORTANT: This key 'document' must match your backend's multer middleware
            // e.g., upload.single('document')
            dataToSave.append('document', documentFile);
        }
        onSave(dataToSave);
    };
                                ...
                                <form onSubmit={handleSubmit} className="mt-6 space-y-6 max-h-[75vh] overflow-y-auto pr-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input id="dueDate" name="dueDate" type="date" value={formData.dueDate} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm" required />
                                        </div>
                                        <div>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-2">Tender Document (PDF, DOC, DOCX)</label>
                                        <div className="mt-2 flex items-center gap-4 p-2 rounded-lg border border-gray-200">
                                            <div className="flex-grow">
                                                {documentFile ? (
                                                    ...
                                                    <a href={`http://localhost:3000${documentUrl}`} target="_blank" rel="noopener noreferrer" className="text-sm p-2 text-blue-600 hover:underline">View current document. Upload to replace.</a>
                                                ) : (
                                            ...
                                            <button type="button" onClick={() => fileInputRef.current.click()} className="flex-shrink-0 flex items-center gap-2 px-3 py-2 text-sm font-semibold text-slate-700 bg-slate-100 rounded-md hover:bg-slate-200">
                                                <ArrowUpTrayIcon className="w-4 h-4" /> {documentFile || documentUrl ? 'Replace' : 'Upload'}
                                            </button>
                                        </div>
